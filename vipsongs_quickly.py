...
async def get_video_url(id, session):
    ...
    title_params['id'] = id
    async with session.get(title_url, headers=headers, params=title_params) as res:
        dom = await res.text()
        name = re.findall(r'<title>(.*?)</title>', dom)[0]
        name = name.replace(" ", "").replace("\n", "").replace("/", ";")

    async with session.post(url, headers=headers, data=data, params=params, cookies=cookies) as res:
